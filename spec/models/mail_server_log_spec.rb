...
describe MailServerLog do
  ...
  describe '#delivery_status' do
    ...
    context 'there is not a stored value' do
      ...
      it 'attempt to parse the line text' do
        log = MailServerLog.new(:line => "hi")
        expect(log).to receive(:line)
        log.delivery_status
      end
      ...
      context 'using the :exim MTA' do
        ...
        it 'returns a delivery status for the log line' do
          log = MailServerLog.new(:line => line, :order => 1)
          status = MailServerLog::EximDeliveryStatus.new(:message_arrival)
          expect(log.delivery_status).to eq(status)
        end

        it 'stores the delivery status value' do
          log = MailServerLog.new(:line => line, :order => 1)
          status = MailServerLog::EximDeliveryStatus.new(:message_arrival)
          log.delivery_status
